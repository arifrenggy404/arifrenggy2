            ...
            <div style="display:flex; flex-direction:column; gap: 1.25rem;">
                @forelse($featuredProjects as $project)
                    <div style="border-bottom: 1px solid var(--border); padding-bottom: 1rem;">
                        <a href="{{ route('projects.show', $project->slug) }}" style="color:var(--text-primary); font-weight:600; font-size:1rem; text-decoration:none; display:flex; justify-content:space-between; align-items:center;">
                            <span>{{ $project->title }}</span>
                            <span style="font-size:0.85rem; color:var(--accent-solid); font-weight: bold;">&rarr;</span>
                        </a>
                        <p style="color:var(--text-secondary); font-size:0.85rem; margin-top:0.35rem; line-height: 1.5;">{{ $project->summary }}</p>
                    </div>
                @empty
